...
class gmwfb_registerhook
{
	public static function gmwfb_activation()
	{
        ...
        if($missingtables) 
		{
			$errors[] = __('These tables could not be created on installation ' . implode(', ',$missingtables), GMWFB_TDOMAIN);
            $has_errors=true;
        }
        ...
        if($has_errors) 
		{
			wp_die( __( $errors[0] , GMWFB_TDOMAIN ) );
			return false;
		}
		else
		{
	...
	public static function gmwfb_adminmenu()
	{
		if (is_admin()) 
		{
			add_options_page( __('Google Map', GMWFB_TDOMAIN), 
				__('Google Map', GMWFB_TDOMAIN), 'manage_options', GMWFB_PLUGIN_NAME, array( 'gmwfb_intermediate', 'gmwfb_admin' ) );
		}		
	}
...
class gmwfb_widget_register extends WP_Widget 
{
	...
	{
		$widget_ops = array('classname' => 'widget_text gmwfb_widget', 'description' => __(GMWFB_PLUGIN_DISPLAY, GMWFB_TDOMAIN), GMWFB_PLUGIN_NAME);
		parent::__construct(GMWFB_PLUGIN_NAME, __(GMWFB_PLUGIN_DISPLAY, GMWFB_TDOMAIN), $widget_ops);
	}
	...
	function form( $instance ) 
	{
		...
		?>
		<p>
		<label for="<?php echo $this->get_field_id('gmwfb_title'); ?>"><?php _e('Widget Title', GMWFB_TDOMAIN); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('gmwfb_id'); ?>"><?php _e('Map Id', GMWFB_TDOMAIN); ?></label>
